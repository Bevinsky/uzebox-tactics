...
//                  DrawMap2((vramX+5)%0x1F, 5, INTERFACE_TR)
                ...
                if(curInput&BTN_A && !(prevInput&BTN_A)) {
                    ...
                    if(selectionVar == 1) { // move
                        ...
                        if(!HASATTACKED(unitList[levelBuffer[cursorX][cursorY].unit].other)) {
                            ...
                            attackedUnit = getNextAttackableUnitIndex(0xFF, DIR_UP);
            ...
            case unit_attack:
                if(curInput&BTN_UP && !(prevInput&BTN_UP)) {
                    attackedUnit = getNextAttackableUnitIndex(attackedUnit, DIR_UP);
                    cursorX = unitList[attackedUnit].xPos;
                    cursorY = unitList[attackedUnit].yPos;
                    moveCursorInstant(cursorX, cursorY);
                }
                if(curInput&BTN_DOWN && !(prevInput&BTN_DOWN)) {
                    attackedUnit = getNextAttackableUnitIndex(attackedUnit, DIR_DOWN);
                    cursorX = unitList[attackedUnit].xPos;
                    cursorY = unitList[attackedUnit].yPos;
                    moveCursorInstant(cursorX, cursorY);
                }
                if(curInput&BTN_LEFT && !(prevInput&BTN_LEFT)) {
                    attackedUnit = getNextAttackableUnitIndex(attackedUnit, DIR_LEFT);
                    cursorX = unitList[attackedUnit].xPos;
                    cursorY = unitList[attackedUnit].yPos;
                    moveCursorInstant(cursorX, cursorY);
        }
                if(curInput&BTN_RIGHT && !(prevInput&BTN_RIGHT)) {
                    attackedUnit = getNextAttackableUnitIndex(attackedUnit, DIR_RIGHT);
                    cursorX = unitList[attackedUnit].xPos;
                    cursorY = unitList[attackedUnit].yPos;
                    moveCursorInstant(cursorX, cursorY);
        }
...
unsigned char getNextAttackableUnitIndex(unsigned char last, char dir) {
        ...
        unsigned char enemyPlayer = (activePlayer == PL1) ? PL2 : PL1; // reverse the player
        unsigned char nextUnit = 0xFF; //default to no unit
        unsigned char nextUnitRollover = 0xFF;
    unsigned char nextUnitRollover2 = 0xFF;

        for(uint8_t i = 0; i < MAX_UNITS; i++) {
            if(
            i != last &&
                unitList[i].isUnit && 
                GETPLAY(unitList[i].info) == enemyPlayer && 
                (
                    (range == 1 && ABS(unitList[i].xPos - unitList[attackingUnit].xPos) == 1 && ABS(unitList[i].yPos - unitList[attackingUnit].yPos) == 1) ||
                    (MANH(unitList[i].xPos, unitList[i].yPos, unitList[attackingUnit].xPos, unitList[attackingUnit].yPos) <= range)
                )
              ) { //if unit exists and on the enemy team and is in range
                ...
                switch(dir) {
                    case DIR_UP:
                        ...
                        if(
                            (last == 0xFF) ||
                            ((unitList[i].yPos > unitList[last].yPos) && 
                            ( ((nextUnit == 0xFF || unitList[i].xPos < unitList[nextUnit].xPos) && unitList[i].xPos >= unitList[last].xPos) || ((nextUnit == 0xFF || unitList[i].xPos == unitList[nextUnit].xPos) && unitList[i].yPos < unitList[nextUnit].yPos) ) )
                        )
                            nextUnit = i;
                            ...
                            (last == 0xFF) ||
                            ((unitList[i].yPos <= unitList[last].yPos) && 
                            ( ((nextUnitRollover == 0xFF || unitList[i].xPos < unitList[nextUnitRollover].xPos) && unitList[i].xPos >= unitList[last].xPos) || ((nextUnitRollover == 0xFF || unitList[i].xPos == unitList[nextUnitRollover].xPos) && unitList[i].yPos < unitList[nextUnitRollover].yPos)  ))
                        )
                            nextUnitRollover = i;

                        if(nextUnitRollover2 == 0xFF || unitList[i].xPos < unitList[nextUnitRollover2].xPos || (unitList[i].xPos == unitList[nextUnitRollover2].xPos && unitList[i].yPos < unitList[nextUnitRollover2].yPos) )
                            nextUnitRollover2 = i;
                ...
                case DIR_LEFT:
                        if(
                            (last == 0xFF) ||
                            ((unitList[i].xPos < unitList[last].xPos) && 
                            (nextUnit == 0xFF || ABS(unitList[last].yPos - unitList[i].yPos) < ABS(unitList[last].yPos - unitList[nextUnit].yPos) || (unitList[i].yPos == unitList[nextUnit].yPos && unitList[i].xPos < unitList[nextUnit].xPos) ) )
                        )
                            nextUnit = i;
                            ...
                            (last == 0xFF) ||
                            ((unitList[i].xPos > unitList[last].xPos) && 
                            (nextUnit == 0xFF || ABS(unitList[last].yPos - unitList[i].yPos) < ABS(unitList[last].yPos - unitList[nextUnitRollover].yPos) || (unitList[i].yPos == unitList[nextUnitRollover].yPos && unitList[i].xPos < unitList[nextUnitRollover].xPos) ) )
                        )
                            nextUnitRollover = i;
    ...
    if(nextUnit != 0xFF)
        return nextUnit;
    else if(nextUnitRollover != 0xFF)
        return nextUnitRollover;
    else if(nextUnitRollover2 != 0xFF)
        return nextUnitRollover2;
    else
        return last;
} //close function
/*
